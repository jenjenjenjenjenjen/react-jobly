...
class JoblyApi {
  ...
  static async getAllCompanies(name) {
    let res = await this.request(name ? `companies?name=${name}` : `companies`)
    return res.companies;
  }

  static async getAllJobs(title) {
    let res = await this.request(title ? `jobs?title=${title}` : 'jobs')
    return res.jobs;
  }

  static async getToken(username, password) {
    let res = this.request(`auth/token`, {username, password}, "post");
    return res;
  }
